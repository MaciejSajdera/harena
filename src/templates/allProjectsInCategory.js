...
class allProjectsInCategory extends React.Component {
  constructor(props) {
    super(props)
  ...
  render() {
    ...
    return (
      <>
      {console.log(this.props.pageContext.categorySlug)}
        <main className={`all-grid`}>
          <div className="all-grid-bg">
            {projects.nodes
              ...
              .filter(node => 
                node.projectCategory === this.props.pageContext.categorySlug
              )
              .map((element, index) => (
                <AniLink
                  cover
               ...
               `}
                  ...
                  key={index}
                  css={{
                    background: `url(${element.fullScreenPhoto.fluid.src})`,
...
allProjectsInCategory.contextType = myContext

export default allProjectsInCategory

export const query = graphql`
  query allProjectsDataCategory($locale: String!) {
